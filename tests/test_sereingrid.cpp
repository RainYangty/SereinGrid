...
void test_negative_coordinates()
{
    ...
    expect_near(grid.get_value(-12, -25), 15.5f, 1e-5f, "negative coordinate value was not preserved");
    expect_near(grid.get_value(-12, -24), 0.0f, 1e-5f, "neighboring coordinate should remain empty");
}
...
void test_coordinate_conversion()
{
    ...
    expect(U == -2 && V == -3 && u == 8 && v == 5, "global_to_local returned unexpected coordinates");

    grid.macro_to_global(U, V, x, y);
    expect(x == -20 && y == -30, "macro_to_global returned unexpected macro origin");
...
void test_add_and_pruning()
{
    ...
    expect_near(grid.get_value(5, 5), 0.0f, 1e-6f, "adding the inverse value should clear the point");
}
...
void test_grid_merge()
{
    ...
    expect_near(target.get_value(0, 0), 2.0f, 1e-5f, "merge should preserve existing target values");
    expect_near(target.get_value(5, 5), 3.0f, 1e-5f, "merge should apply the requested coordinate offset");
}
...
void test_moved_from_macro_node_can_be_reused()
{
    ...
    expect(assigned_destination.fine_grid != nullptr, "move assignment lost the destination fine grid");
    expect(assigned_source.fine_grid != nullptr, "move-assigned source was not reusable");
}
} // namespace
